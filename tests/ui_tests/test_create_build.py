...
class TestCreateBuild(BaseUiTest):
    ...
    def test_user_creates_build(self, test_data):
        ...
        with allure.step("Open `Create Build Page` (http://localhost:8111/admin/createObjectMenu.html)"):
            create_build_page = BuildCreatePage.open(driver = self.driver, project_id=test_data.project.id)
            time.sleep(15)
        ...
        with allure.step("Check that build type was successfully created with correct data on API level"):
            time.sleep(10)
            build_request = CheckedRequest(self.specifications.authSpec(test_data.user), Endpoint.BUILD_TYPES.url)
        ...
        with allure.step("Check that build is visible on build page (http://localhost:8111/buildConfiguration/{build_id})"):
            build_page = BuildPage.open(self.driver, test_data.project.id, test_data.buildtype.name)
    ...
    def test_user_creates_build_without_name(self, test_data):
        ...
        with allure.step(
            ...
            build_request = UncheckedRequest(self.specifications.authSpec(test_data.user), Endpoint.BUILD_TYPES.url)
            created_build = build_request.read(f'name:{test_data.buildtype.name}')
