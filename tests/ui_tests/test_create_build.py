...
class TestCreateBuild():
    ...
    def test_user_creates_build(self, test_data, user_factory, specifications, driver):
        ...
        with allure.step("Check that build type was successfully created with correct data on API level"):
            time.sleep(10)
            build_request = CheckedRequest(specifications.authSpec(user), Endpoint.BUILD_TYPES.url)
    ...
    def test_user_creates_build_without_name(self, test_data, user_factory, specifications, driver):
        ...
        with allure.step(
            ...
            build_request = UncheckedRequest(specifications.authSpec(user), Endpoint.BUILD_TYPES.url)
            created_build = build_request.read(f'name:{test_data.buildtype.name}')
